    ...
    for(int hh = 0; hh < h_subtile/2; hh++) {
      for(int ww = 0; ww < w_subtile/2; ww++) {
        output_access(item_channel_index,(item_row_index / 2 + hh),(item_col_index / 2 + ww)) = max(
          max(
            max(0.f,c_access(0,(hh * 2),(ww * 2    )) + bias[item_channel_index]), max(0.f,c_access(0,(hh * 2 + 1),(ww * 2    )) + bias[item_channel_index])
          ), 
          max(
            max(0.f,c_access(0,(hh * 2),(ww * 2 + 1)) + bias[item_channel_index]), max(0.f,c_access(0,(hh * 2 + 1),(ww * 2 + 1)) + bias[item_channel_index])
          )
        );
        output_access(item_channel_index+1,(item_row_index / 2 + hh),(item_col_index / 2 + ww)) = max(
          max(
            max(0.f,c_access(1,(hh * 2),(ww * 2    )) + bias[item_channel_index+1]), max(0.f,c_access(1,(hh * 2 + 1),(ww * 2    )) + bias[item_channel_index+1])
          ), 
          max(
            max(0.f,c_access(1,(hh * 2),(ww * 2 + 1)) + bias[item_channel_index+1]), max(0.f,c_access(1,(hh * 2 + 1),(ww * 2 + 1)) + bias[item_channel_index+1])
          )
        );
        output_access(item_channel_index+2,(item_row_index / 2 + hh),(item_col_index / 2 + ww)) = max(
          max(
            max(0.f,c_access(2,(hh * 2),(ww * 2    )) + bias[item_channel_index+2]), max(0.f,c_access(2,(hh * 2 + 1),(ww * 2    )) + bias[item_channel_index+2])
          ), 
          max(
            max(0.f,c_access(2,(hh * 2),(ww * 2 + 1)) + bias[item_channel_index+2]), max(0.f,c_access(2,(hh * 2 + 1),(ww * 2 + 1)) + bias[item_channel_index+2])
          )
        );
        output_access(item_channel_index+3,(item_row_index / 2 + hh),(item_col_index / 2 + ww)) = max(
          max(
            max(0.f,c_access(3,(hh * 2),(ww * 2    )) + bias[item_channel_index+3]), max(0.f,c_access(3,(hh * 2 + 1),(ww * 2    )) + bias[item_channel_index+3])
          ), 
          max(
            max(0.f,c_access(3,(hh * 2),(ww * 2 + 1)) + bias[item_channel_index+3]), max(0.f,c_access(3,(hh * 2 + 1),(ww * 2 + 1)) + bias[item_channel_index+3])
          )
        );
        output_access(item_channel_index+4,(item_row_index / 2 + hh),(item_col_index / 2 + ww)) = max(
          max(
            max(0.f,c_access(4,(hh * 2),(ww * 2    )) + bias[item_channel_index+4]), max(0.f,c_access(4,(hh * 2 + 1),(ww * 2    )) + bias[item_channel_index+4])
          ), 
          max(
            max(0.f,c_access(4,(hh * 2),(ww * 2 + 1)) + bias[item_channel_index+4]), max(0.f,c_access(4,(hh * 2 + 1),(ww * 2 + 1)) + bias[item_channel_index+4])
          )
        );
        output_access(item_channel_index+5,(item_row_index / 2 + hh),(item_col_index / 2 + ww)) = max(
          max(
            max(0.f,c_access(5,(hh * 2),(ww * 2    )) + bias[item_channel_index+5]), max(0.f,c_access(5,(hh * 2 + 1),(ww * 2    )) + bias[item_channel_index+5])
          ), 
          max(
            max(0.f,c_access(5,(hh * 2),(ww * 2 + 1)) + bias[item_channel_index+5]), max(0.f,c_access(5,(hh * 2 + 1),(ww * 2 + 1)) + bias[item_channel_index+5])
          )
        );
        output_access(item_channel_index+6,(item_row_index / 2 + hh),(item_col_index / 2 + ww)) = max(
          max(
            max(0.f,c_access(6,(hh * 2),(ww * 2    )) + bias[item_channel_index+6]), max(0.f,c_access(6,(hh * 2 + 1),(ww * 2    )) + bias[item_channel_index+6])
          ), 
          max(
            max(0.f,c_access(6,(hh * 2),(ww * 2 + 1)) + bias[item_channel_index+6]), max(0.f,c_access(6,(hh * 2 + 1),(ww * 2 + 1)) + bias[item_channel_index+6])
          )
        );
        output_access(item_channel_index+7,(item_row_index / 2 + hh),(item_col_index / 2 + ww)) = max(
          max(
            max(0.f,c_access(7,(hh * 2),(ww * 2    )) + bias[item_channel_index+7]), max(0.f,c_access(7,(hh * 2 + 1),(ww * 2    )) + bias[item_channel_index+7])
          ), 
          max(
            max(0.f,c_access(7,(hh * 2),(ww * 2 + 1)) + bias[item_channel_index+7]), max(0.f,c_access(7,(hh * 2 + 1),(ww * 2 + 1)) + bias[item_channel_index+7])
          )
        );
      }
    }
